...
def main():
    ...
    #切换为tensorboard
    if accelerator.is_main_process:
        tb_dir = Path(config.experiment.output_dir) / "tensorboard_logs1"
    ...
    #纠正tokenization mis
    tokenizer = AutoTokenizer.from_pretrained(config.model.mmada.pretrained_model_path, padding_side="left",use_fast=True)
    ...
    if config.dataset.recon_type == "journeydb":
        ...
        from data_reca import MidjourneyDataset  
        #the path to dataset
        dataset = load_dataset("/data2/midjourney-v6-llava/")['train']
    ...
    #     from data_reca import get_instruct_data_loader
    ...
    if config.experiment.resume_from_checkpoint:
        ...
        if path is not None and os.path.exists(path):
            ...
            global_step = int(str(path).split("-")[-1])
            first_epoch = global_step // num_update_steps_per_epoch
    ...
    #构造重建训练的输入和标签
    @torch.no_grad()
    def prepare_inputs_and_labels_for_recon(
        pixel_values_or_image_ids: Union[torch.FloatTensor, torch.LongTensor],
        texts: Union[str, str],
        min_masking_rate: float = 0.0,
        is_train: bool = True,
        ...
        input_ids_recon,masks_recon,labels_recon = uni_prompting((texts, input_ids, resized_image_tokens, labels), 't2i_recon')

        return input_ids_recon, labels_recon, mask_prob, image_tokens, masks_recon
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            pixel_values_recon = batch["recon_flow"]["images"].to(accelerator.device)
            texts_recon = batch["recon_flow"]["input_ids"]
            ...
            if config.training.recon_coeff > 0:
                (
                    ...
                    recon_masks
                ) = prepare_inputs_and_labels_for_recon(pixel_values_recon, [f"USER: \n { random.choice(recon_prompt_list) } ASSISTANT:"] * pixel_values_recon.shape[0], config.training.min_masking_rate, is_train=True, resized_pixel_values=resized_pixel_values)
                #hello: torch.Size([2, 1541]) torch.Size([2, 3057])
                # print("hello:",input_ids_recon.shape, labels_recon.shape)
                print("input_ids_recon",input_ids_recon)
                exit(0)
            ...
            if "llava" in config.dataset.und_type:
                #input_ids_mmu是否经过mask
                pixel_values_mmu, input_ids_mmu, labels_mmu, prompt_masks_mmu_ori = (batch["mmu_flow"]["images"], batch["mmu_flow"]["input_ids"],batch["mmu_flow"]["labels"], batch["mmu_flow"]["prompt_masks"])
                ...
                prompt_masks_mmu_ori = prompt_masks_mmu_ori.to(accelerator.device, non_blocking=True)
                ...
                #构造prompt mask
                #1.前缀长度
                prefix_len = 1+1+image_tokens_mmu.shape[1]+1  #<|mmu|> + <|soi|> + image tokens + <|eoi|>
                prompt_masks = torch.cat([torch.ones(input_ids_mmu.shape[0], prefix_len,dtype=torch.long).to(accelerator.device), prompt_masks_mmu_ori.long()], dim=1)
                assert prompt_masks.shape == input_ids_mmu.shape, \
                    f"prompt_masks shape {prompt_masks.shape} != input_ids shape {input_ids_mmu.shape}"
                print(f"prompt_masks shape {prompt_masks.shape} != input_ids shape {input_ids_mmu.shape}")
                (
                    input_ids_mmu,  
                    labels_mmu,
                    p_mask_mmu,
            ...
            # print(input_ids.shape,input_ids_mmu.shape)
            ...
            with accelerator.accumulate(model):
                #注释mmu和t2i联合训练部分
                if config.training.recon_coeff > 0 or config.training.mmu_coeff > 0:
                    logits, loss_t2i, ____, loss_mmu = model.forward_process(
                        input_ids=input_ids,
                        labels=labels,
                    ...
                    logits = None
                    loss_t2i = torch.tensor(0.0, device=accelerator.device)
                    loss_mmu = torch.tensor(0.0, device=accelerator.device)
                ...
                #图像重建损失
                # print(recon_masks.shape)
                if config.training.recon_coeff > 0:
                    loss_recon,logits = model.forward_process_recon(
                    ...
                    loss_recon = torch.tensor(0.0, device=accelerator.device)
                ...
                #修改为只训练recon，调试
                # loss = config.training.recon_coeff * loss_recon
                avg_masking_rate = accelerator.gather(mask_prob_recon.repeat(config.training.batch_size_recon)).mean()
                accelerator.backward(loss)
                if accelerator.is_main_process:
                    writer.add_scalar("loss_recon", avg_loss_recon.item(), global_step)
                    print(f"Step {global_step}: loss_recon={loss_recon.item():.4f}, total_loss={loss.item():.4f}, masking_rate={avg_masking_rate.item():.3f}")
...
def visualize_predictions(
    ...
    input_ids = input_ids[:config.training.batch_size_recon, -(config.model.mmada.num_vq_tokens + 1):-1:] - len(uni_prompting.text_tokenizer)
    ...
    combined_tensor = torch.from_numpy(predicted_images).permute(0, 3, 1, 2)
    # import torchvision
    # grid = torchvision.utils.make_grid(combined_tensor, nrow=1, padding=10, pad_value=255)
    ...
    # writer.add_image(
    #     "Original v.s. Reconstructed v.s. Predicted images",
    #     grid,
...
def reconstruct_images(
    ...
    logger.info("reconstruct images...")
    model.eval()
