...
from training.data import Text2ImageDataset, MyParquetDataset
...
def main():
    ...
    elif config.dataset.gen_type == "t2i-parquet":
        dataset = MyParquetDataset(
            root_dir='/data/dataset/meissonic-aesthetics'
        )
        print('Length of parquet dataset: ', len(dataset))
        train_dataloader_t2i = DataLoader(
            dataset,
            batch_size=config.training.batch_size_t2i, # per gpu batch size
            shuffle=False,
            num_workers=dataset_config.num_workers,
            collate_fn=default_collate,
            pin_memory=True,
        )
        train_dataloader_t2i.num_batches = len(train_dataloader_t2i)
    ...
    if config.dataset.gen_type != "t2i-parquet":
        model, optimizer, lr_scheduler = accelerator.prepare(model, optimizer, lr_scheduler)
    else:
        model, optimizer, lr_scheduler, combined_dataloader = accelerator.prepare(
            model, optimizer, lr_scheduler, combined_dataloader
        )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            if global_step == 0 and epoch == 0:
                logger.info("Input ids: {}".format(input_ids.shape))
                logger.info("Labels: {}".format(labels.shape))

            with accelerator.accumulate(model):
                loss_t2i = model.forward_t2i(
                    input_ids=input_ids,
                    labels=labels,
...
def generate_images(
    ...
    with torch.autocast("cuda", dtype=weight_dtype, enabled=accelerator.mixed_precision != "no"):
        # Generate images
        gen_token_ids = accelerator.unwrap_model(model).t2i_generate_mcat(
            input_ids=input_ids,
    ...
    logger.info("Images generated, check it out on wandb!")
