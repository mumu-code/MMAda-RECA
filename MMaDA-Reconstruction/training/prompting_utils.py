...
class UniversalPrompting():
    ...
    def t2i_recon_prompt(self, text_ids, image_ids, given_image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            if self.max_text_len >= len(temp_ids_):
                old_len = len(temp_ids_)
                temp_masks = [0] * (self.max_text_len - old_len-1) + [1] * (old_len + len(given_image_ids[i]) +image_ids.shape[-1] + 5)
            else:
                temp_masks = [1] * (len(temp_ids_) + len(given_image_ids[i]) + image_ids.shape[-1] + 5)  # +2 for two special tokens

            pad_ids = [self.pad_id] * (self.max_text_len - len(temp_ids_) - 1)

            #同Show-o不同，MMADA中attentionmask文本和图像都采用双向注意力机制
            ...
            temp_label_ids = torch.cat([
              (torch.ones_like(torch.tensor(pad_ids)) * self.ignore_id).to(device),
                ...
                torch.tensor([self.ignore_id]).to(device),
                labels[i],  # the actual label for the image tokens. size: [batchsize,num_image_tokens]
                torch.tensor([self.ignore_id]).to(device),
            ], dim=0)
            temp_masks = torch.tensor(temp_masks).to(device)
    ...
    def t2i_recon_gen_prompt(self, text_ids, image_ids, given_image_ids):
        ...
        return torch.cat(sequence_ids, dim=0), torch.cat(attention_masks,dim=0), None
